...
import com.example.sendmessage.DeviceListFragment.DeviceActionListener;
import com.example.sendmessage.QuizApp.PTPLog;

public class QuizActivity extends Activity implements DeviceActionListener {
    ...
    public void onResume() {
        ...
        if( mApp.mThisDevice != null ){
        	...
        	// if p2p connetion info available, and my status is connected
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            ...
            case R.id.atn_direct_discover:
                if( !mApp.isP2pEnabled() ){
                    Toast.makeText(QuizActivity.this, R.string.p2p_off_warning, Toast.LENGTH_LONG).show();
                    return true;
                }
                ...
                mApp.mP2pMan.discoverPeers(mApp.mP2pChannel, new WifiP2pManager.ActionListener() {
                    ...
                    public void onSuccess() {
                        Toast.makeText(QuizActivity.this, "Discovery Initiated", Toast.LENGTH_SHORT).show();
                    ...
                    public void onFailure(int reasonCode) {
                    	...
                    	fragment.clearPeers();
                        Toast.makeText(QuizActivity.this, "Discovery Failed, try again... ", Toast.LENGTH_SHORT).show();
                    }
                });
                return true;
    ...
    public void connect(WifiP2pConfig config) {
        ...
        mApp.mP2pMan.connect(mApp.mP2pChannel, config, new ActionListener() {
            ...
            public void onSuccess() {
            	...
            	Toast.makeText(QuizActivity.this, "Connect success..", Toast.LENGTH_SHORT).show();
            }

            @Override
            public void onFailure(int reason) {
                Toast.makeText(QuizActivity.this, "Connect failed. Retry.", Toast.LENGTH_SHORT).show();
            }
        });
    }
    ...
    public void disconnect() {
        ...
        mApp.mP2pMan.removeGroup(mApp.mP2pChannel, new ActionListener() {
            ...
            public void onFailure(int reasonCode) {
                ...
                Toast.makeText(QuizActivity.this, "disconnect failed.." + reasonCode, Toast.LENGTH_SHORT).show();
            }
        ...
        if (mApp.mP2pMan != null) {
            ...
            if (fragment.getDevice() == null || fragment.getDevice().status == WifiP2pDevice.CONNECTED) {
                ...
                mApp.mP2pMan.cancelConnect(mApp.mP2pChannel, new ActionListener() {
                    @Override
                    public void onSuccess() {
                        Toast.makeText(QuizActivity.this, "Aborting connection", Toast.LENGTH_SHORT).show();
                        PTPLog.d(TAG, "cancelConnect : success canceled...");
                    }
                    @Override
                    public void onFailure(int reasonCode) {
                        Toast.makeText(QuizActivity.this, "cancelConnect: request failed. Please try again.. ", Toast.LENGTH_SHORT).show();
     ...
     * launch quiz
     */
    public void startQuizActivity(final String initMsg) {
    	if( ! mApp.mP2pConnected ){
    		Log.d(TAG, "startQuizActivity : p2p connection is missing, do nothng...");
    		return;
    	}
    	
    	PTPLog.d(TAG, "startQuizActivity : start quiz activity fragment..." + initMsg);
    	runOnUiThread(new Runnable() {
    		@Override public void run() {
    			//Intent i = mApp.getLauchActivityIntent(MainActivity.class, initMsg);
    	    	//startActivity(i);
    			//Intent i = mApp.getLauchActivityIntent(Questions.class, initMsg);
    			 PTPLog.d(TAG, "my address" + mApp.mMyAddr);
    			
    			Intent i= new Intent(QuizActivity.this, MainQuiz.class);
    	    	startActivity(i);
