...
import static com.example.sendmessage.Constants.*;

import com.example.sendmessage.AppPreferences;
...
public class ConnectionService extends Service implements ChannelListener, PeerListListener, ConnectionInfoListener {  // callback of requestPeers{
    ...
    /**
     * process all WI-FI p2p intent caught by broadcast receiver.
    ...
    private void processIntent(Intent intent){
    	if( intent == null){
    		...
    		PTPLog.d(TAG, "processIntent: " + intent.toString()+"action"+ action);
    ...
    /**
     * send a notification upon receive data, click the notification will broadcast the pending intent, which
     * will launch the quiz activity fragment.
     */
    ...
    private void showInActivity(final MessageRow row){
    	...
    	if( mActivity != null ){
    		...
    		if( mApp.mHomeActivity != null && mApp.mHomeActivity.mHasFocus == true ){
    			...
    			mApp.mHomeActivity.startQuizActivity(row.mMsg);
    		}else{
