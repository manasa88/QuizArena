...
public class ConnectionManager {
	...
	// Server knows all clients. key is ip address, value is socket channel. 
	// when remote client screen on, a new connection with the same ip address is established.
	 ...
	 */
		public ConnectionManager(ConnectionService service) {
			mService = service;
			mApp = (QuizApp)mService.getApplication();
		}
	 ...
	 */
		public int startServerSelector() {
			...
			try {	
				...
				Log.d(TAG, "startServerSelector : address " + mServerAddr);
				if( "192.168.49.1".equals(mServerAddr)){
					mServerAddr = "Master";
				}
				((QuizApp)mService.getApplication()).setMyAddr(mServerAddr);
	...
	public void onFinishConnect(SocketChannel schannel){
		...
		((QuizApp)mService.getApplication()).setMyAddr(mClientAddr);
	}
